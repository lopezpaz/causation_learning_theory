...
def f1(x,K):
  r = np.ones((x.shape[0],K))
  for i in range(K):
    r[:,i] = np.power(x,i+1).ravel()
  return r

  return np.cos(np.dot(np.hstack((x,np.ones((x.shape[0],1)))),w))
def f2(x,y,z):
  return np.hstack((f1(x,K).mean(0),f1(y,K).mean(0)))#,f1(z,wz).mean(0)))
...
def tuebingen(fx="/agbs/datasets/causal_pairs_20000/tuebingen_pairs.csv",fy="/agbs/datasets/causal_pairs_20000/tuebingen_target.csv"):
  f  = open(fx); pairs = f.readlines(); f.close();
  z1 = np.zeros((len(pairs),2*K))
  z2 = np.zeros((len(pairs),2*K)) 
  i  = 0
  for row in pairs:
    r       = row.split(",",2)
