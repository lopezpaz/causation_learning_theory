...
def graph(n=1000,k=2,p1=2,p2=1,v=3,d=5):
...
def abalone(reg,fx=ABALONE_X):
  ...
  for i in range(d):
    x = X[:,i][:,np.newaxis]
    #xx = np.sqrt(2./np.median(np.power(x-x[np.random.permutation(x.shape[0]),:],2)))
    for j in range(i+1,d):
      y = X[:,j][:,np.newaxis]
      for k in list(set(range(d)) - set([i,j])):
        z  = X[:,k][:,np.newaxis]
        #zz = np.hstack((x,y,z))
        #zz = np.sqrt(2./np.median(np.sum(np.power(zz-zz[np.random.permutation(zz.shape[0]),:],2),1)))
        #print [xx,zz]
